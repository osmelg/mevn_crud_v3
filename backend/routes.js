    ...
    // AUTENTICACION DE USUARIOS
        // ACCESO 
            router.post('/login',(req,res)=>{
                ...
                .exec()
                .then(usuario =>{
                    if (usuario.length < 1){res.status(401).json({rs:'emailIncorrecto'})}
                    // 2. Comparar contraseña con respecto a mongo
                    bcrypt.compare(req.body.password,usuario[0].password,(err,usuarioEncontrado)=>{
                        if (err){res.status(401).json({rs: 'errorIncriptacion'})
                        }
                        // *-* se puede hacer else o else if?
